...
def add_is_compulsory_stop():
    ...
    # Edge case: If percentage of compulsory stops os 0, there is still one time window in the data

    global route_nodes

    percentage_of_compulsory_stops = data['parameters']['compulsory_percentage']

    for entry in route_nodes:
        for node in data['time_windows']:
            if entry['node_id'] == node['node_id'] and percentage_of_compulsory_stops != 0:
...
def add_is_origin_and_is_destination():
    ...
    origin_node_id = data['time_windows'][0]['node_id']
    destination_node_id = data['time_windows'][len(data['time_windows']) - 1]['node_id']

    for entry in route_nodes:
        if entry['node_id'] == origin_node_id:
            entry['is_origin'] = True
        else:
            entry['is_origin'] = False

        if entry['node_id'] == destination_node_id:
