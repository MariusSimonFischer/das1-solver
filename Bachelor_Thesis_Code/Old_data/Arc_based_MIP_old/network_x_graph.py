...
class Stop:
    def __init__(self, latitude, longitude, is_compulsory_stop, order, segment_h, is_origin, is_destination):
...
def initialize_stops():
    ...
    compulsory_stops = [
        Stop(node['latitude'], node['longitude'], node['is_compulsory_stop'], node['order'], node['segment_h'],
             node['is_origin'], node['is_destination']) for node
...
def build_graph(compulsory_stops: list, optional_stops: list) -> nx.Graph:
    ...
    graph_for_segment = [nx.DiGraph() for i in range(len(tb.bounds) - 1)]
            ...
            for j in graph.nodes():
                if i != j:
                    graph.add_edge(i, j, weight=dict_map.distance_matrix[i.order][j.order],
                                   time=tm.time_matrix[i.order][j.order],
                                   segment=n + 1)
