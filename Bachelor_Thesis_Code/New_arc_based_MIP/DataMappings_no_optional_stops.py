...
def get_cpst_id():
    """
    Function to get the id of the compulsory stops
    :return: list of ids of the compulsory stops
    """
    cpst_id = []
    for entry in data['time_windows']:
        cpst_id.append(entry['node_id'])
    return cpst_id
...
def find_min_distance_and_node():
    ...
    for entry in data['buildings_distance_matrix']:
        ...
        for i in range(len(entry)):
            if i == 0:
                list_cpst_ids = get_cpst_id()
                # Check whether the node is a compulsory stop
                if data['route_nodes'][i]['node_id'] in list_cpst_ids:
