...
def build_graph(compulsory_stops: list, optional_stops: list) -> nx.Graph:
    ...
    for n in range(len(graph_for_segment)):
        ...
        relevant_compulsory_stops = [node for node in compulsory_stops if n + 1 in node.segment_h]
        ...
        for i in graph.nodes():
            for j in graph.nodes():
                if i != relevant_compulsory_stops[1] and j != relevant_compulsory_stops[0] and i != j:
